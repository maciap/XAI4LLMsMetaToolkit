...
def _neuronpedia_layer_id(release: str, sae_id: str) -> Optional[str]:
    """
    Map (release, sae_id) to Neuronpedia layer id.

    For GPT-2 'res-jb' SAEs hosted on Neuronpedia:
      blocks.<L>.hook_resid_pre  ->  "<L>-res-jb"

    Return None if we don't know the mapping (UI can hide Neuronpedia embeds).
    """
    if release == "gpt2-small-res-jb":
        try:
            parts = sae_id.split(".")
            layer = int(parts[1])
            return f"{layer}-res-jb"
        except Exception:
            return None

    # Add more mappings later when you confirm Neuronpedia IDs for other releases
    return None
...
def _neuronpedia_feature_url(
    ...
    """
    Build an embeddable Neuronpedia dashboard URL for a single feature.
    """
    base = f"https://neuronpedia.org/{model_id}/{layer_id}/{feature_idx}"
    params = {
        "embed": "true",
    ...
    if default_text:
        params["defaulttesttext"] = default_text
...
class SAEFeatureExplorer:
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if device == "cuda" and not torch.cuda.is_available():
            raise ValueError("CUDA requested, but torch.cuda.is_available() is False (CPU-only PyTorch or no GPU).")
        ...
        # --- Neuronpedia embeds (Level 1): only if we can map this SAE to a Neuronpedia layer id ---
        neuronpedia: Dict[str, Any] = {"enabled": False}
        layer_id = _neuronpedia_layer_id(release=release, sae_id=sae_id)
        if layer_id is not None:
            neuronpedia["enabled"] = True
            neuronpedia["model_id"] = model_name
            neuronpedia["layer_id"] = layer_id
            neuronpedia["feature_urls"] = [
                {
                    "feature_id": tf["feature_id"],
                    "url": _neuronpedia_feature_url(
                        model_id=model_name,
                        layer_id=layer_id,
                        feature_idx=int(tf["feature_id"]),
                        default_text=text,
                    ),
                }
                for tf in top_features[: min(10, len(top_features))]
            ]
        ...
        return {
            ...
            "notes": [
                ...
                "Neuronpedia embeds are provided when this SAE release/id can be mapped to a hosted Neuronpedia dashboard.",
            ],
        }
