...
# -------------------------
# IMPORTANT PATCH:
# typeguard's @typechecked breaks on jaxtyping dimension strings like "batch pos"
# by trying to AST-parse them. For deployment/debug UI we don't need runtime
# typechecking, so we make it a no-op.
# -------------------------
def typechecked(obj):
    return obj
...
class TransformerLensTransparentLlm(TransparentLlm):
    ...
    def decomposed_ffn_out(
        ...
        return torch.mul(
            processed_activations.unsqueeze(-1),
            self._model.blocks[layer].mlp.W_out,
        )
